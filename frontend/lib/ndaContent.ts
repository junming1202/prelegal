// Bundled Mutual NDA content and formatting helpers.
//
// The Standard Terms below are copied verbatim from the Common Paper Mutual
// Non-Disclosure Agreement, Version 1.0 (templates/Mutual-NDA.md), free to use
// under CC BY 4.0 (https://creativecommons.org/licenses/by/4.0/).
//
// Formatting helpers convert raw form data into the human-readable strings used
// by BOTH the on-screen preview and the generated PDF, keeping them identical.

import type { NdaFormData } from "./ndaTypes";

/** A single numbered clause of the MNDA Standard Terms. */
...
/** Placeholder shown in the preview when a field has not been filled in yet. */
const placeholder = (value: string, fallback: string): string => {
  const trimmed = value.trim();
  return trimmed ? trimmed : `[${fallback}]`;
};

/**
 * Number of years for a term, defaulting to "1" if the user has cleared the
 * field — an empty count would otherwise render as a placeholder in the final
 * legal document.
 */
const years = (value: string): string => {
  const trimmed = value.trim();
  return trimmed ? trimmed : "1";
};

/** A labelled value for rendering the cover page as key/value rows. */
export interface CoverPageField {
  label: string;
  value: string;
}

/** Build the displayable cover-page fields from raw form data. */
export function buildCoverPageFields(data: NdaFormData): CoverPageField[] {
  const mndaTerm =
    data.mndaTermType === "expires"
      ? `Expires ${years(data.mndaTermYears)} year(s) from the Effective Date.`
  ...
  const confidentialityTerm =
    data.confidentialityTermType === "years"
      ? `${years(
        ...
        )} year(s) from the Effective Date, but in the case of trade secrets until the Confidential Information is no longer considered a trade secret under applicable laws.`
      : "In perpetuity.";

  return [
    { label: "Purpose", value: placeholder(data.purpose, "How Confidential Information may be used") },
    { label: "Effective Date", value: placeholder(data.effectiveDate, "Effective Date") },
    { label: "MNDA Term", value: mndaTerm },
    { label: "Term of Confidentiality", value: confidentialityTerm },
    { label: "Governing Law", value: placeholder(data.governingLaw, "State") },
    { label: "Jurisdiction", value: placeholder(data.jurisdiction, "City or county and state") },
    {
      label: "MNDA Modifications",
      value: data.modifications.trim() ? data.modifications.trim() : "None.",
    },
  ];
}

/** Rows for the signature block, one labelled entry per party value. */
...
export function buildSignatureRows(data: NdaFormData): SignatureRow[] {
  const dash = "—";
  return [
    {
      label: "Print Name",
      party1: placeholder(data.party1.printName, dash),
      party2: placeholder(data.party2.printName, dash),
    },
    {
      label: "Title",
      party1: placeholder(data.party1.title, dash),
      party2: placeholder(data.party2.title, dash),
    },
    {
      label: "Company",
      party1: placeholder(data.party1.company, dash),
      party2: placeholder(data.party2.company, dash),
    },
    {
      label: "Notice Address",
      party1: placeholder(data.party1.noticeAddress, dash),
      party2: placeholder(data.party2.noticeAddress, dash),
    },
  ];
}
